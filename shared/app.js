...
// Shared demonstration behavior for all three complete layout themes. No backend or real member data.
const iconPaths = {
...
const demoState = window.NorthLionsDemoState;
const demoRegistrations = new Set(demoState ? demoState.registrationIds : ['meeting', 'service']);
let priorFocus = null;
let toastTimer;
...
function openDialog(html, kicker = window.NorthLionsThemes?.themes.find(theme => theme.id === document.documentElement.dataset.siteTheme)?.dialogKicker || "MEMBER'S LOUNGE") {
...
document.addEventListener('click', (event) => {
  ...
  if (panelButton && panels[panelButton.dataset.panel]) openDialog(panels[panelButton.dataset.panel]());
    ...
    if (demoState) demoState.register(key);
    document.querySelector('.member-stats button strong').innerHTML = `${String(demoRegistrations.size).padStart(2, '0')}<span>場</span>`;
    document.querySelectorAll(`.event-card[data-event="${key}"] .event-status`).forEach((status) => {
...
document.querySelectorAll('[data-filter]').forEach((button) => {
  button.addEventListener('click', () => {
    if (demoState) demoState.setFilter(button.dataset.filter);
...
// Render the same demonstration state in each layout's existing components.
document.querySelector('.member-stats button strong').innerHTML = `${String(demoRegistrations.size).padStart(2, '0')}<span>場</span>`;
document.querySelectorAll('.event-card[data-event] .event-status').forEach(status => {
  const joined = demoRegistrations.has(status.closest('[data-event]').dataset.event);
  status.textContent = joined ? '已加入' : '開放報名'; status.classList.toggle('open', !joined);
});
if (demoState) document.querySelector(`[data-filter="${demoState.filter}"]`)?.click();
